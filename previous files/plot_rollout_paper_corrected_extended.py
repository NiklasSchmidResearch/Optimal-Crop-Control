...
# 1. Run a 'dry pass' to extract violations without generating plots
_, _, _, is_violating_3 = plot_results_for_one_controller("3_rollout_objects.pkl", dry_run=True)
_, _, _, is_violating_36 = plot_results_for_one_controller("36_rollout_objects.pkl", dry_run=True)

# 2. Find a specific trajectory index where 3-day violates, but 36-day does not
found_target_idx = -1
for idx, (viol_3, viol_36) in enumerate(zip(is_violating_3, is_violating_36)):
    if viol_3 and not viol_36:
        found_target_idx = idx
        break

if found_target_idx != -1:
    print(
        f"\nSUCCESS: Found trajectory index {found_target_idx} where the 3-day controller violates, but the 36-day controller is safe.")
else:
    print(
        "\nWARNING: Could not find a trajectory that violates in 3-day but not in 36-day. Defaulting highlight index to -1.")
