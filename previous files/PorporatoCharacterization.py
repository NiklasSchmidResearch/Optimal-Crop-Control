...
# Assuming x and net_flux_to_mineral_nitrogen_gN_per_m3_per_day are already defined numpy arrays
# For demonstration, let's assume they are defined as follows:
# x = ...
# net_flux_to_mineral_nitrogen_gN_per_m3_per_day = ...
...
df = pd.DataFrame(
    {
        "Time_years": x,
        "Net_flux_gN_per_m3_per_day": net_flux_to_mineral_nitrogen_gN_per_m3_per_day,
    }
)

# Sparsify the data: Keep every 10th row (adjust the step size as needed to reduce density)
df_sparse = df.iloc[::50].reset_index(drop=True)

# 1. Export the sparsified data to a CSV file
df_sparse.to_csv("mineral_nitrogen_flux.csv", index=False)

# 2. Print a sample of the data formatted as a LaTeX table
df_table_sample = df_sparse.head(50)

latex_table = df_table_sample.to_latex(
    index=False,
    header=["Time (years)", "Net Flux (gN/m$^3$/day)"],
    caption="Sparsified net flux to mineral nitrogen under agricultural conditions.",
    label="tab:nitrogen_flux_sparse",
    column_format="cc",
    position="h!",
)
